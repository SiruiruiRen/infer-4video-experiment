...
CREATE TABLE IF NOT EXISTS reflections (
    ...
    feedback_raw TEXT,           -- Raw LLM response (before formatting)
    revision_number INTEGER DEFAULT 1,
    parent_reflection_id UUID,   -- For tracking revision chains
    revision_time_seconds NUMERIC, -- Time since previous revision (null for first revision)
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
-- Scores are percentages (0-100+) that can add up to over 100%
-- ============================================================================
CREATE TABLE IF NOT EXISTS binary_classifications (
    ...
    window_id TEXT,              -- Window identifier from analysis
    window_text TEXT,
    description_score NUMERIC,   -- Percentage score for Description (0-100+)
    explanation_score NUMERIC,  -- Percentage score for Explanation (0-100+)
    prediction_score NUMERIC,   -- Percentage score for Prediction (0-100+)
...
-- event_data JSONB stores detailed information:
--   - view_feedback_start/end: {style: 'extended'|'short', duration_seconds: NUMERIC}
--   - concept_explanation_clicked: {concept_name: TEXT, concept_type: TEXT}
--   - revision events: {revision_number: INTEGER, time_since_last: NUMERIC}
-- ============================================================================
CREATE TABLE IF NOT EXISTS user_events (
    ...
    event_data JSONB DEFAULT '{}',  -- Stores all event-specific data
...
-- view_feedback_start: When user starts viewing feedback (stores style: 'extended'|'short')
-- view_feedback_end: When user stops viewing feedback (stores duration_seconds, style)
-- select_feedback_style: When user switches between extended/short
-- copy_feedback: When user copies feedback text
-- concept_explanation_clicked: When user clicks to view concept explanations (stores concept_name, concept_type)
-- revision_started: When user starts a revision (stores revision_number, time_since_last_revision)
-- final_submission: When user makes final submission for a video
-- survey_completed: When user completes a survey
-- reading_time_extended: Time spent reading extended feedback (duration_seconds)
-- reading_time_short: Time spent reading short feedback (duration_seconds)
